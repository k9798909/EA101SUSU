...
//				}
		...
		if("tryLogin".equals(action)) {// 來自loginBack.jsp的請求
			...
			try {
				...
				if(!errorMsgs.isEmpty()) {
					RequestDispatcher failView = req.getRequestDispatcher("/back-end/loginBack.jsp");
					failView.forward(req, res);
					return;
				}
				...
				/***************************3.查詢完成,準備轉交(Send the Success view)***********/
					...
					errorMsgs.add(e.getMessage());
					RequestDispatcher failureView = req.getRequestDispatcher("/back-end/loginBack.jsp");
		...
		if("changePwd".equals(action)) {// 來自updatePwd.jsp的請求
			...
			try{
				...
				errorMsgs.add(e.getMessage());
				String url = "/back-end/loginBack.jsp";
				RequestDispatcher successView = req.getRequestDispatcher(url);
				successView.forward(req, res);
			}
		}
